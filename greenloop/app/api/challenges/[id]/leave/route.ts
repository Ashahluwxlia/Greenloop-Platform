...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    console.log(`[v0] Leave challenge request - User: ${user.id}, Challenge: ${challengeId}`)
    ...
    console.log(`[v0] Participation check - Found: ${!!participation}, Error: ${participationError?.message}`)
    ...
    console.log(
      `[v0] Attempting to delete participation record: ${participation.id} for user: ${user.id} in challenge: ${challengeId}`,
    )
    ...
    console.log(`[v0] Delete operation - Error: ${deleteError?.message || "none"}, Deleted count: ${deletedCount}`)
    ...
    console.log(`[v0] Verification after deletion - records found: ${count}, data: ${JSON.stringify(verifyDeleted)}`)
